...
int ossl_ec_GFp_simple_tpl(const EC_GROUP *group, EC_POINT *r, const EC_POINT *a,
                           BN_CTX *ctx)
{
    int (*field_mul)(const EC_GROUP *, BIGNUM *, const BIGNUM *,
                     const BIGNUM *, BN_CTX *);
    ...
    BIGNUM *temp1 = BN_CTX_get(ctx);
    BIGNUM *temp2 = BN_CTX_get(ctx);
    BIGNUM *temp3 = BN_CTX_get(ctx);
    BIGNUM *temp4 = BN_CTX_get(ctx);
    BIGNUM *temp5 = r->Z; // BN_CTX_get(ctx);
    BIGNUM *temp6 = r->X; // BN_CTX_get(ctx);
    BIGNUM *temp7 = r->Y; // BN_CTX_get(ctx);
    ...
    if (a == r)
    {
        temp_aX = BN_CTX_get(ctx);
        temp_aY = BN_CTX_get(ctx);
        temp_aZ = BN_CTX_get(ctx);
        BN_copy(temp_aX, a->X);
        BN_copy(temp_aY, a->Y);
        BN_copy(temp_aZ, a->Z);
    }
    ...
    field_sqr(group, XX, temp_aX, ctx);
    // YY=Y1^2
    BIGNUM *YY = BN_CTX_get(ctx);
    field_sqr(group, YY, temp_aY, ctx);
    // ZZ=Z1^2
    BIGNUM *ZZ = BN_CTX_get(ctx);
    field_sqr(group, ZZ, temp_aZ, ctx);

    // YYYY=YY^2
    BIGNUM *YYYY = BN_CTX_get(ctx);
    field_sqr(group, YYYY, YY, ctx);

    // M=3*XX+a*ZZ^2
    BIGNUM *M = BN_CTX_get(ctx);
    field_sqr(group, temp1, ZZ, ctx);
    field_mul(group, temp2, group->a, temp1, ctx);
    BN_mod_lshift1_quick(temp3, XX, p);
    BN_mod_add_quick(temp4, temp3, XX, p);
    BN_mod_add_quick(M, temp4, temp2, p);

    // MM=M^2
    BIGNUM *MM = BN_CTX_get(ctx);
    field_sqr(group, MM, M, ctx);

    // E=6*((X1+YY)^2-XX-YYYY)-MM
    BIGNUM *E = BN_CTX_get(ctx);
    BN_mod_add_quick(temp1, temp_aX, YY, p);
    field_sqr(group, temp2, temp1, ctx);
    BN_mod_sub_quick(temp3, temp2, XX, p);
    BN_mod_sub_quick(temp4, temp3, YYYY, p);
    BN_mod_lshift1_quick(temp5, temp4, p);
    BN_mod_lshift1_quick(temp6, temp5, p);
    BN_mod_add_quick(temp7, temp5, temp6, p);
    BN_mod_sub_quick(E, temp7, MM, p);

    // EE=E^2
    BIGNUM *EE = BN_CTX_get(ctx);
    field_sqr(group, EE, E, ctx);

    // T=16*YYYY
    BIGNUM *T = BN_CTX_get(ctx);
    BN_mod_lshift_quick(T, YYYY, 4, p);

    // U=(M+E)^2-MM-EE-T
    BIGNUM *U = BN_CTX_get(ctx);
    BN_mod_add_quick(temp1, M, E, p);
    field_sqr(group, temp2, temp1, ctx);
    BN_mod_sub_quick(temp3, temp2, MM, p);
    BN_mod_sub_quick(temp4, temp3, EE, p);
    BN_mod_sub_quick(U, temp4, T, p);

    // X3=4*(X1*EE-4*YY*U)
    field_mul(group, temp1, temp_aX, EE, ctx);
    field_mul(group, temp2, YY, U, ctx);
    BN_mod_lshift_quick(temp3, temp2, 2, p);
    BN_mod_sub_quick(temp4, temp1, temp3, p);
    BN_mod_lshift_quick(r->X, temp4, 2, p);

    // Y3=8*Y1*(U*(T-U)-E*EE)
    field_mul(group, temp1, E, EE, ctx);
    BN_mod_sub_quick(temp2, T, U, p);
    field_mul(group, temp3, U, temp2, ctx);
    BN_mod_sub_quick(temp4, temp3, temp1, p);
    field_mul(group, temp5, temp_aY, temp4, ctx);
    BN_mod_lshift_quick(r->Y, temp5, 3, p);

    // Z3=(Z1+E)^2-ZZ-EE
    BN_mod_add_quick(temp1, temp_aZ, E, p);
    field_sqr(group, temp2, temp1, ctx);
    BN_mod_sub_quick(temp3, temp2, ZZ, p);
    BN_mod_sub_quick(r->Z, temp3, EE, p);
                                         ...
                                         BN_CTX *ctx)
{
    ...
    if ((group->meth->field_encode != NULL && !group->meth->field_encode(group, lambda, lambda, ctx)) || !group->meth->field_mul(group, p->Z, p->Z, lambda, ctx) || !group->meth->field_sqr(group, temp, lambda, ctx) || !group->meth->field_mul(group, p->X, p->X, temp, ctx) || !group->meth->field_mul(group, temp, temp, lambda, ctx) || !group->meth->field_mul(group, p->Y, p->Y, temp, ctx))
        goto end;
                                  ...
                                  EC_POINT *p, BN_CTX *ctx)
{
    ...
    if (!p->Z_is_one /* r := 2p */
        || !group->meth->field_sqr(group, t3, p->X, ctx) || !BN_mod_sub_quick(t4, t3, group->a, group->field) || !group->meth->field_sqr(group, t4, t4, ctx) || !group->meth->field_mul(group, t5, p->X, group->b, ctx) || !BN_mod_lshift_quick(t5, t5, 3, group->field)
        /* r->X coord output */
        || !BN_mod_sub_quick(r->X, t4, t5, group->field) || !BN_mod_add_quick(t1, t3, group->a, group->field) || !group->meth->field_mul(group, t2, p->X, t1, ctx) || !BN_mod_add_quick(t2, group->b, t2, group->field)
        /* r->Z coord output */
    ...
    if (!group->meth->field_mul(group, r->Z, r->Z, r->Y, ctx) || !group->meth->field_mul(group, r->X, r->X, r->Y, ctx) || !group->meth->field_mul(group, s->X, p->X, s->Z, ctx)) /* s := p */
        return 0;
                                   ...
                                   EC_POINT *p, BN_CTX *ctx)
{
    ...
    if (t6 == NULL || !group->meth->field_mul(group, t6, r->X, s->X, ctx) || !group->meth->field_mul(group, t0, r->Z, s->Z, ctx) || !group->meth->field_mul(group, t4, r->X, s->Z, ctx) || !group->meth->field_mul(group, t3, r->Z, s->X, ctx) || !group->meth->field_mul(group, t5, group->a, t0, ctx) || !BN_mod_add_quick(t5, t6, t5, group->field) || !BN_mod_add_quick(t6, t3, t4, group->field) || !group->meth->field_mul(group, t5, t6, t5, ctx) || !group->meth->field_sqr(group, t0, t0, ctx) || !BN_mod_lshift_quick(t2, group->b, 2, group->field) || !group->meth->field_mul(group, t0, t2, t0, ctx) || !BN_mod_lshift1_quick(t5, t5, group->field) || !BN_mod_sub_quick(t3, t4, t3, group->field)
        /* s->Z coord output */
        || !group->meth->field_sqr(group, s->Z, t3, ctx) || !group->meth->field_mul(group, t4, s->Z, p->X, ctx) || !BN_mod_add_quick(t0, t0, t5, group->field)
        /* s->X coord output */
        || !BN_mod_sub_quick(s->X, t0, t4, group->field) || !group->meth->field_sqr(group, t4, r->X, ctx) || !group->meth->field_sqr(group, t5, r->Z, ctx) || !group->meth->field_mul(group, t6, t5, group->a, ctx) || !BN_mod_add_quick(t1, r->X, r->Z, group->field) || !group->meth->field_sqr(group, t1, t1, ctx) || !BN_mod_sub_quick(t1, t1, t4, group->field) || !BN_mod_sub_quick(t1, t1, t5, group->field) || !BN_mod_sub_quick(t3, t4, t6, group->field) || !group->meth->field_sqr(group, t3, t3, ctx) || !group->meth->field_mul(group, t0, t5, t1, ctx) || !group->meth->field_mul(group, t0, t2, t0, ctx)
        /* r->X coord output */
        || !BN_mod_sub_quick(r->X, t3, t0, group->field) || !BN_mod_add_quick(t3, t4, t6, group->field) || !group->meth->field_sqr(group, t4, t5, ctx) || !group->meth->field_mul(group, t4, t4, t2, ctx) || !group->meth->field_mul(group, t1, t1, t3, ctx) || !BN_mod_lshift1_quick(t1, t1, group->field)
        /* r->Z coord output */
