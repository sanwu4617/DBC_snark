...
__device__ void sha256_init(MYSHA256_CTX *ctx);
__device__ void sha256_update(MYSHA256_CTX *ctx, const BYTE data[], size_t len);
__device__ void sha256_final(MYSHA256_CTX *ctx, BYTE hash[]);
...
void print_hashs(UINT8 *hashs,int n){
	for(int i=0;i<n;i++){
		printhash(hashs+i*256);
	}
		// printf("%s \n", hash_to_string(hashs+i*256));
}
...
__device__ void sha256_transform(MYSHA256_CTX *ctx, const BYTE data[])
{
