...
uint288::uint288(BIGNUM* n)
{
    BN_CTX* ctx=BN_CTX_new();
    BIGNUM* pow2_32=BN_new();
    BIGNUM* temp=BN_new();
    BIGNUM* temp2=BN_new();
    BN_copy(temp2,n);
    BN_set_word(pow2_32,65536);
    BN_mul_word(pow2_32,65536);
    for (int i = 0; i < 9; i++)
    {
        BN_div(temp2, temp, temp2, pow2_32, ctx);
        data[8 - i] = BN_get_word(temp);
    }
}
